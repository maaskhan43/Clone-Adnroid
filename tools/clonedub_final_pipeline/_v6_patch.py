# -*- coding: utf-8 -*-
# patch-run for V05,V08,V15,V19,V20: relaxed accept (speed<=1.15, +0.3s grace),
# no start-trim under 1.2s, extra short tiers.
...
TXT={5:["फिर से तुम लोग?","फिर तुम लोग?"],
     8:["दाओमिंग-सी सीनियर, मैं साफ़ कर देती हूँ।","सीनियर, मैं साफ़ कर देती हूँ।"],
     15:["तुम..."],
     19:["झेगेर की थाली? शानचाई, तुम...","झेगेर की थाली? शानचाई..."],
     20:["थाली कैसे मिली? ज़रूर उसी वजह से।","थाली उसी वजह से मिली होगी।"]}
...
for i,L in enumerate(lines):
    ...
    texts=TXT[vid]; made=0
    for c in range(3):
        hi=texts[min(c//2+ (1 if made==0 and c==2 else 0),len(texts)-1)] if len(texts)>1 else texts[0]
        audio=model(hi,ref_audio_path=rw,ref_text=rt)
        a=np.array(audio,dtype=np.float32)
        if a.size<SR//8: 
            print("  V%02d_c%d empty"%(vid,c),flush=True); continue
        if np.max(np.abs(a))>1.5: a=a/32768.0
        a=a/(np.max(np.abs(a))+1e-9)*0.9
        a=start_trim(a,SR)
        gen=len(a)/SR
        if avail>0.4 and gen>avail*1.15+0.3:
            print("  V%02d_c%d too long %.1f"%(vid,c,gen),flush=True); continue
        ...
        made+=1
        print("  V%02d_c%d final=%.2f avail=%.2f sp=%.2f | %s"%(vid,c,len(a)/SR,avail,f,hi[:22]),flush=True)
json.dump(rep,open(W+"/v6_gen_report.json","w",encoding="utf-8"),indent=1,ensure_ascii=False)
print("V6_PATCH_DONE",flush=True)
